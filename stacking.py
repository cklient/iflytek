...
stack=pd.read_csv('../data/stacking.csv')


# In[ ]:


use_all=data.merge(stack,on='instance_id',how='left')


# In[ ]:


use_all=use_all[use_all['train_1'].notnull()]


# In[ ]:


use_all_ind=list(use_all.index)
...
all_df=pd.concat([all_df,use_all.iloc[:,-4:]],axis=1)
...
train_x =all_df[:len(train)]
train_y = data[data.target != -1].target.values
test_x=all_df[len(train):]
...
skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=argsDict["rands"])
tmprmselist=[]
treerec=[]
...
print(log_loss(train_y,oof_predss))


# In[ ]:


sub=use_all[len(train):][['instance_id']]
...
sub.to_csv('../subs/10171.csv',index=None)
